...
def _executor_thread_ident(provider: ScriptedProvider) -> int:
    """Identify the single worker thread backing the offload seam."""
    return provider.offload(threading.get_ident).result()
...
def test_async_entry_runs_the_driver_off_the_awaiting_thread() -> None:
    class _ThreadRecordingProvider(ScriptedProvider):
    ...
    worker_thread = _executor_thread_ident(provider)
...
def test_async_entry_does_not_block_the_event_loop() -> None:
    ...
    class _LoopGatedProvider(ScriptedProvider):
        def invoke(
            self, request: ProviderCallRequest
        ) -> ProviderInvocationEvidence:
            offload_started.set()
...
def test_cancelling_the_task_does_not_interrupt_a_running_offload() -> None:
    ...
    class _GatedProvider(ScriptedProvider):
        def invoke(
            self, request: ProviderCallRequest
        ) -> ProviderInvocationEvidence:
            started.set()
...
def test_cancelled_await_reports_an_orphaned_offload_failure(
    ...
    class _RaisingProvider(ScriptedProvider):
        def __init__(self) -> None:
            super().__init__()
